...
namespace raresy {
    ...
    * @note This class does not own the memory of the Fields.
     ...
     *  [{"ops2", StatusCode::ERR_}, {"ops5", StatusCode::ERR_}, {...}, ...]
     *
     ...
     * unless the overall code is `ERR_MULTIPLE_OPERATIONS_FAILED`, in which case... good luck, I guess.
     *
    ...
    public:
         ...
         *
         * FIELD_TARGET is of type `ResponseField` and is usually of the intended type `TARGET_TYPE`. However,
         * since `ResponseField` can also be `FIELD_TYPE`, it is also supported programmatically.
         */
    ...
    private:
        ...
        constexpr bool handle_overflow() noexcept {
            // NOTE: _failure_count is pre-incremented and represents the 1-based human count.
    ...
    public:
        ...
        void addErrorEntry(F field_target, StatusCode error_code) noexcept {
            ...
            // "Construct" the pair at the `failures` position (`failure_count`).
